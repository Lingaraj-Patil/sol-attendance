...
import { Building2, AlertCircle, CheckCircle, Briefcase, Clock, GraduationCap, User, BookOpen } from 'lucide-react';
...
const ConnectInstitute = () => {
  ...
  const handleStudentChange = (e) => {
    ...
    if (name === 'maxCourses') {
      ...
      const newSelected = Array(numCourses).fill(null).map((_, i) => currentSelected[i] || '');
  ...
  const handleTeacherSubmit = async (e) => {
    ...
    try {
      if (!teacherFormData.collegeUniqueId || !teacherFormData.experience || !teacherFormData.department || !teacherFormData.workingHour) {
        setError('Please fill in all required fields.');
        setLoading(false);
        return;
      }

      if (isNaN(teacherFormData.experience) || parseFloat(teacherFormData.experience) < 0) {
        setError('Years of Experience must be a non-negative number.');
        setLoading(false);
        return;
      }

      if (isNaN(teacherFormData.workingHour) || parseFloat(teacherFormData.workingHour) <= 0) {
      ...
      if (token) {
        ...
        setMessage({ type: 'success', text: 'Teacher registration completed successfully! Redirecting to dashboard...' });
        setTimeout(() => {
          navigate('/dashboard');
        }, 1500);
      } else {
  ...
  const handleStudentSubmit = async (e) => {
    ...
    try {
      if (!studentFormData.collegeUniqueId || !studentFormData.age || !studentFormData.gender || !studentFormData.Program) {
        setError('Please fill in all required fields (College Unique ID, Age, Gender, Program).');
        setLoading(false);
        return;
      }
      ...
      if (studentFormData.maxCourses && (isNaN(studentFormData.maxCourses) || parseFloat(studentFormData.maxCourses) < 1)) {
      ...
      const studentData = {
        ...
        ...(studentFormData.maxCourses && { maxCourses: parseInt(studentFormData.maxCourses) }),
      ...
      if (token) {
        ...
        setMessage({ type: 'success', text: 'Student registration completed successfully! Redirecting to dashboard...' });
        setTimeout(() => {
          navigate('/dashboard');
        }, 1500);
      } else {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-primary-50 to-primary-100 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-2xl w-full space-y-8">
        ...
        <form 
          ...
          onSubmit={userType === 'teacher' ? handleTeacherSubmit : handleStudentSubmit}
        >
          {error && (
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                type="text"
                required
                value={userType === 'teacher' ? teacherFormData.collegeUniqueId : studentFormData.collegeUniqueId}
                onChange={userType === 'teacher' ? handleTeacherChange : handleStudentChange}
          ...
          {userType === 'teacher' && (
            <>
              ...
              <div>
                <label htmlFor="workingHour" className="block text-sm font-medium text-gray-700 mb-2">
          ...
          {userType === 'student' && (
            <>
              ...
              <div>
                <label htmlFor="maxCourses" className="block text-sm font-medium text-gray-700 mb-2">
              ...
              {studentFormData.maxCourses && parseInt(studentFormData.maxCourses) > 0 && (
                <div>
                  ...
                  {loadingCourses ? (
                    ...
                    <div className="bg-primary-50 border border-primary-200 rounded-lg p-4">
                      <p className="text-sm text-primary-800">
                        No courses available. Please enter a valid College Unique ID or contact your administrator.
                      </p>
                    </div>
                  ) : (
                    <div className="space-y-3">
                      {Array.from({ length: parseInt(studentFormData.maxCourses) || 0 }).map((_, index) => (
                        <div key={index}>
                          <label className="block text-xs font-medium text-gray-600 mb-1">
                            Course {index + 1} *
                          </label>
                          <select
                            value={studentFormData.selectedCourses[index] || ''}
                          ...
                          >
                            ...
                            {availableCourses.map((course) => {
                              ...
                              const isAlreadySelected = studentFormData.selectedCourses.some(
                                (selected, idx) => selected === courseId && idx !== index
                              );
                              return (
                                <option
                                  key={courseId}
          ...
          <div className="flex space-x-4">
            ...
            <button
              type="button"
              onClick={() => {
                localStorage.removeItem(userType === 'teacher' ? 'teacherInfo' : 'studentInfo');
                navigate('/register');
              }}
