...
export const generateDummyTimetable = async (req, res) => {
  try {
    ...
    if (students.length === 0 || teachers.length === 0 || courses.length === 0) {
      return res.status(400).json({
        success: false,
        message: 'Not enough data to generate timetable. Need at least 1 student, 1 teacher, and 1 course.',
        details: {
          students: students.length,
          teachers: teachers.length,
          courses: courses.length
    ...
    courses.forEach((course, courseIndex) => {
      ...
      for (let i = 0; i < slotsPerCourse && slotIndex < timeSlots.length; i++) {
        ...
        assignments[slot] = {
          ...
          student_groups: course.students?.map(s => s._id?.toString() || s.toString()) || []
        };
    ...
    const inputData = {
      ...
      courses: courses.map((course, index) => ({
        ...
        course_track: 'Major',
        components: { 
          theory: course.credits > 3 ? course.credits - 1 : course.credits || 3, 
          lab: course.credits > 3 ? 1 : 0 
        },
        student_groups: ['G1'],
    ...
    res.json({
      ...
      data: {
        ...
        summary: {
          ...
          students_with_schedules: Object.keys(studentTimetables).length,
          teachers_with_schedules: Object.keys(facultyTimetables).length,
          courses: courses.length,
          students: students.length,
          teachers: teachers.length
        }
    ...
    res.status(500).json({
      success: false,
      message: 'Failed to generate dummy timetable',
      error: error.message
    });
  }
};